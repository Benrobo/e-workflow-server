...
export default function sendMail(res, payload = {}) {
    if (payload.from === undefined || payload.to === undefined) {
        return util.sendJson(
            res,
            {
                error: true,
                message:
                    "Failed to send mail, [ftom, to, payload] params might be missing",
            },
            400
        );
    }
    ...
    if (payload.body === "") {
        return util.sendJson(
            res,
            { error: true, message: "Failed to send mail, payload [body] is empty" },
            400
        );
    }
    ...
    if (Object.entries(payload).length === 0) {
        return util.sendJson(
            res,
            { error: true, message: "Failed to send mail, empty payload" },
            400
        );
    }
    ...
    if (payload.from === "") {
        return util.sendJson(
            res,
            { error: true, message: "Failed to send mail, from is empty" },
            400
        );
    }
    if (payload.to === "") {
        return util.sendJson(
            res,
            { error: true, message: "Failed to send mail, to is empty" },
            400
        );
    }
